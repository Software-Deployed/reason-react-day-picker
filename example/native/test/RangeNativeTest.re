...
let testScenario = (scenario: string) =>
  Alcotest.test_case(scenario, `Quick, () => {
    let cmd = "../RangeNativeRenderer.exe " ++ scenario;
    ...
    | WEXITED(0) => ()
    | WEXITED(code) =>
      Alcotest.fail("RangeNativeRenderer exited with code " ++ string_of_int(code))
    | WSIGNALED(sig_) =>
      Alcotest.fail("RangeNativeRenderer killed by signal " ++ string_of_int(sig_))
    | WSTOPPED(sig_) =>
      Alcotest.fail("RangeNativeRenderer stopped by signal " ++ string_of_int(sig_))
    };
...
let () =
  Alcotest.run(
    "Range Native Renderer Tests",
    [
      (
        "range-scenarios",
        Array.to_list(Array.map(testScenario, RangeScenario.all)),
      ),
    ],
  );
